...
class Class_Name:  # 1단계: 클래스 이름 정의
    ...
    def callback(self, msg):  # 3단계: 클래스 내의 콜백 함수 설정
        # 메세지를 출력합니다.
        print(msg)  # 메세지 전체 출력
        print(msg.orientation)  # 출력 : 메세지 항목(orientation)
        print(msg.linear_acceleration)  # 출력 : 메세지 항목(linear_acceleration)
        print(msg.angular_velocity)  # 출력 : 메세지 항목(angular_velocity)
