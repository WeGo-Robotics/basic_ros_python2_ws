...
# 이 예제를 통해 학생들은 클래스를 사용하여 로봇을 제어하고 무작위로 움직이게 하는 방법을 배울 수 있습니다.

# rospy 라이브러리와 필요한 메시지 유형(geometry_msgs.msg, turtlesim.msg), 무작위 및 수학 함수를 가져옵니다.
import rospy
...
class Class_Name:  # 1단계: 클래스 이름 정의
    def __init__(self):  # 2단계: 클래스 초기화 및 초기 설정
        # ROS 노드를 초기화합니다. 노드 이름은 "wego_node"로 지정됩니다.
    ...
    def callback(self, msg):  # 3단계: 클래스 내의 콜백 함수 설정
        # 메시지 항목(linear.x)을 무작위로 설정합니다.
        self.msg.linear.x = random() * 2  # 메시지 항목(linear.x) - 데이터 값 변경

        # 메시지 항목(angular.z)을 무작위로 설정합니다.
        self.msg.angular.z = random() * 4 - 2  # 메시지 항목(angular.z) - 데이터 값 변경

        # 퍼블리셔를 사용하여 Twist 메시지를 발행합니다.
        self.pub.publish(self.msg)  # ROS 3단계(필수): 퍼블리셔 - 퍼블리시 실행

        # 현재 메시지와 linear.x, angular.z 값을 출력합니다.
        print("msg: ", msg)  # 출력: 메시지
        print("msg.linear.x: ", msg.linear.x)  # 출력: 메시지(linear.x)
        print("msg.angular.z: ", msg.angular.z)  # 출력: 메시지(angular.z)
