...
# 이 예제를 통해 학생들은 클래스를 사용하여 로봇을 제어하고 위치 정보를 구독하며 제어 알고리즘을 이해하는 방법을 배울 수 있습니다.
...
class Class_Name:  # 1단계: 클래스 이름 정의
    def __init__(self):  # 2단계: 클래스 초기화 및 초기 설정
        ...
        rospy.init_node("wego_node")  # ROS 1단계(필수): 노드 이름 정의
    ...
    def callback(self, msg):  # 3단계: 클래스 내의 콜백 함수 설정
        # 메시지 항목(linear.x)을 1로 설정합니다.
        self.msg.linear.x = 1  # 메시지 항목(linear.x) - 데이터 값(1)으로 변경

        # 만약 로봇의 현재 x 위치가 8 이상이면, 메시지 항목(linear.x)을 0으로 설정합니다.
        if msg.x >= 8:
            self.msg.linear.x = 0  # 메시지 항목(linear.x) - 데이터 값(0)으로 변경
